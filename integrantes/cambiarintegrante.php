...
if (PermisoQuery::permisosDeUsuario( $cd_usuario, "Cambiar colaborador" )) {
	...
	$xtpl = new XTemplate ( 'cambiarintegrante.html' );
	...
	$tipoinvestigadores = TipoinvestigadorQuery::listar ($oIntegrante->getCd_tipoinvestigador(), ($oIntegrante->getCd_tipoinvestigador()!=1)?2:0);
	...
	if ($_GET ['er'] == 5) {
			$xtpl->assign ( 'classMsj', 'msjerror' );
			$msj = "Error: Fecha/s de obtención de cargo y/o obtención de beca fuera del período";
			$xtpl->assign ( 'msj', 'onLoad ="mensajeError(\''.$msj.'\')"' );
		}
	if ($_GET ['er'] == 6) {
			$xtpl->assign ( 'classMsj', 'msjerror' );
	...
	$xtpl->assign ( 'titulo', 'SeCyT - Cambiar colaborador' );
