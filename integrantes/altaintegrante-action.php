...
if (PermisoQuery::permisosDeUsuario( $cd_usuario, "Alta integrante" )) {
	...
	if (isset ( $_POST ['bl_estudiante'] )){
		
		$oDocente->setBl_estudiante ( $_POST ['bl_estudiante'] );
	}
	if (isset ( $_POST ['nu_materias'] )){
		$oDocente->setNu_materias(  ( $_POST ['nu_materias'] ) );
	...
	if ((($oIntegrante->getCd_tipoinvestigador()==2)&&((in_array($oCategoria->getDs_categoria(),$categoriasPermitidas)||((in_array($oDocente->getCd_carrerainv(), $carrerainvs))&&($oUnidad->getCd_padre()==1850)))))||($oIntegrante->getCd_tipoinvestigador()!=2)){
		...
		$max = ((in_array($oDocente->getCd_deddoc(), $mayordedicacion))||(in_array($oDocente->getCd_carrerainv(), $carrerainvs)))?1:0;	
		if($oDocente->getBl_becario()){
			$max = 0;
		}
			...
			if ($insertarInt){
				if ($_FILES['ds_curriculum']['tmp_name']) {
			        ...
			        if (!move_uploaded_file($_FILES['ds_curriculum']['tmp_name'], $dir.$nuevo)){
						...
						header ( 'Location: altaintegrante.php?er=4&cd_proyecto=' . $oProyecto->getCd_proyecto().'&dir=' . $dir.$nuevo );
						exit;
			        }
				...
				$dt_alta = date($_SESSION ["nu_yearSessionP"].'-'.$_SESSION ["nu_mesSessionP"].'-01');
				if (( $_POST ['dt_beca'] )&&($_POST ['dt_beca']!='00/00/0000')){
				...
				if ($exito){
					...
					$oIntegrante->setDt_alta ($dt_alta );
					$oIntegrante->setCd_estado(1);
