...
*/
	...
	header ("Pragma: no-cache"); // HTTP/1.0
		...
		$max = (count($aUsers)<=20)?count($aUsers):20;
		for ($i=0;$i<$max;$i++)
		{
			$oUnidad = new  Unidad();
...
$ds_padre = substr($ds_padre,0,strlen($ds_padre)-3);
			echo "<rs id=\"".$aUsers[$i]['cd_unidad']."\" info=\"".utf8_encode(str_replace('"',"'",str_replace(" & "," and ",$ds_padre)))."\" nivel=\"".$aUsers[$i]['bl_hijos']."\">".utf8_encode(str_replace(" & "," and ",$aUsers[$i]['ds_unidad']))."</rs>";
			$ds_padre = '';
		}
		echo "</results>";
